<div x-data="bookNavigation({{ Js::from($books->pluck('id')) }}, {{ $books->search(fn($b) => $b->id === $currentBook->id) }})" 
    @pointerdown.window.passive="start($event)" @pointerup.window.passive="end($event)"
    @touchstart.window.passive="start($event.touches[0])" @touchend.window.passive="end($event.changedTouches[0])"
    @keydown.window="
        if ($event.ctrlKey && $event.shiftKey && $event.key === 'ArrowRight') nextBook();
        if ($event.ctrlKey && $event.shiftKey && $event.key === 'ArrowLeft') prevBook();
    "

    style="touch-action: pan-y;" class="overflow-hidden">
    <div x-ref="container" class="transition-transform duration-300 ease-in-out">
        <!-- Navigation Desktop -->
        <div class="flex justify-between mb-2">
            <!-- Bouton précédent -->
            <button @click="prevBook()" :disabled="currentIndex === 0"
            ...
            <!-- Bouton suivant -->
            <button @click="nextBook()" :disabled="currentIndex === books.length - 1"
        ...
        <!-- Contenu du livre -->
        <div x-bind:class="animating ? (swipeDirection === 'left' ? 'slide-left' : 'slide-right') : ''"
